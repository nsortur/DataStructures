 ...
 * InfAdder.java
 *
 * Takes 2 files of integers(num1.txt and num2.txt), converts the integers from
 * each file into a linked list (makes a number), then adds the two numbers
 * together. It uses a modified digit class attached with this program.
 *
 * @author Neel Sortur
 */
...
//being carried over -see addSame()
	 ...
	 * arrays which will then be inserted into the linked list Sets the starting
	 * values of variables lists) Runs each method in order (inserts into list, adds
	 * the lists, prints the
	 */
...
//Create second list
	...
	/**
	 * If the two lists are the same length, this adds them Like addition on paper,
	 * numbers have to be carried over when they exceed 10. This adds the 'carry'
	 * int to each node so the addition is done properly
	 *
	 ...
	 *        values from the first file into the first list When list equals 2,
	 *        that means the program is inserting the values from the second file
	 *        into the second list When the list equals 3, that means the program is
	 *        adding a carried digit to the front of the sum
	 */
...
//Current object pointer is defined
	 ...
	 * This adds the 'carry' int to each node so the addition is done properly If
	 * there is an extra integer in carried, it adds to the front of the sum
	 *
	 ...
	 * them together using addSame() Takes into account if the lists are different
	 * sizes
	 */
	void addLists() {
